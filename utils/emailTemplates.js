              ...
              <tr>
                ...
                <td>
                  <span style="font-size:20px;font-weight:800;color:${TEXT};letter-spacing:-0.5px;">
                    Launcherdesk <span style="color:${ACCENT};">CRM</span>
                  </span>
                </td>
              </tr>
            </table>
        ...
        <tr>
          <td style="padding:18px 40px;border-top:1px solid ${BORDER};">
            <p style="margin:0;font-size:11px;color:${DIM};text-align:center;">
              © ${YEAR} Launcherdesk CRM &nbsp;·&nbsp; Automated system email — please do not reply
            </p>
          </td>
        </tr>
...
const companyWelcomeEmail = ({
  companyName,
  plan = "Basic",
  loginUrl     = process.env.FRONTEND_URL   || "https://app.launcherdesk.com",
  supportEmail = process.env.SUPPORT_EMAIL  || "support@launcherdesk.com",
}) => {
...
const subscriptionExpiryEmail = ({
  ...
  daysLeft,
  expiryDate,
  renewUrl     = process.env.FRONTEND_URL   ? `${process.env.FRONTEND_URL}/billing` : "https://app.launcherdesk.com/billing",
  supportEmail = process.env.SUPPORT_EMAIL  || "support@launcherdesk.com",
}) => {
  const planLabel   = plan.charAt(0).toUpperCase() + plan.slice(1);
  ...
  const subject = daysLeft === 1
    ? `URGENT: Your Launcherdesk CRM plan expires TODAY — ${companyName}`
    : `Your Launcherdesk CRM subscription expires in ${dayLabel} — ${companyName}`;

  return {
    subject,
    html: shell(body),
    text: `Launcherdesk CRM Subscription Expiry Notice\n\nHi ${companyName},\n\nYour ${planLabel} plan expires on ${expiryStr} (${dayLabel} left).\n\nRenew at: ${renewUrl}\n\nAfter expiry your account will switch to read-only mode.\n\nNeed help? Email ${supportEmail}.`,
  };
};
...
const superAdminExpiryDigestEmail = ({
  ...
  dashboardUrl   = process.env.FRONTEND_URL
    ...
    : "https://app.launcherdesk.com/developer/subscriptions",
}) => {
  ...
  // Plain-text fallback
  const lines = [`Launcherdesk CRM — SuperAdmin Subscription Expiry Digest`, ``, `Hi ${superAdminName},`, `Run at: ${runDate}`, ``, `${totalExpiring} company/companies require attention.`, ``];
  if (hasCritical) {
  ...
  return {
    subject: totalExpiring === 0
      ? `✅ No Expiring Subscriptions Today — Launcherdesk CRM`
      : hasCritical
        ? `🚨 ${expiringGroups.critical.length} Subscription(s) Expiring TODAY — Launcherdesk CRM`
        : `⚠️ ${totalExpiring} Subscription(s) Expiring Soon — Launcherdesk CRM`,
    html: shell(body),
    text: lines.join("\n"),
  };
};
...
const planInvoiceEmail = ({
  ...
  const text = [
    `Launcherdesk CRM \u2014 Plan ${actionWord} ${isPaid ? "Invoice" : "Notification"}`,
    ``,
    `Hi ${recipientName},`,
...
const addonInvoiceEmail = ({
  ...
  dashboardUrl   = process.env.FRONTEND_URL
    ...
    : "https://app.launcherdesk.com/billing",
}) => {
  ...
  const text = [
    `Launcherdesk CRM \u2014 Add-on ${actionWord} ${isPaid ? "Invoice" : "Notification"}`,
    ``,
    `Hi ${recipientName},`,
