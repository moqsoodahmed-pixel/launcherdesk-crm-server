...
const sendEmail = async ({ to, toName, subject, html, text }) => {
  ...
  const fromEmail = process.env.BREVO_FROM_EMAIL || "noreply@launcherdesk.com";
  const fromName  = process.env.BREVO_FROM_NAME  || "Launcherdesk CRM";
      ...
      <table width="480" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:32px 40px 24px;border-bottom:1px solid #1E2130;">
            <span style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#F59E0B;">Super Admin · Launcherdesk CRM</span>
          </td>
        </tr>
        <!-- Body -->
        <tr>
        ...
        <tr>
          <td style="padding:20px 40px;border-top:1px solid #1E2130;">
            <p style="margin:0;font-size:11px;color:#3A3F52;text-align:center;">
              © ${new Date().getFullYear()} Launcherdesk CRM · Automated security email
            </p>
          </td>
        </tr>
      </table>
...
<body style="margin:0;padding:0;background:#0D0F14;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0D0F14;padding:40px 16px;">
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:32px 40px 24px;border-bottom:1px solid #1E2130;">
            <span style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#3B82F6;">${roleLabel} · Launcherdesk CRM</span>
          </td>
        </tr>
        <!-- Body -->
        <tr>
  ...
  const text = `Launcherdesk CRM — Password Reset OTP\n\nHi ${toName || roleLabel},\n\nYour one-time password to reset your account password: ${otp}\n\nExpires in 10 minutes. Single use only.\n\nIf you did not request this, please ignore this email.`;

  return sendEmail({
    to:      toEmail,
    toName,
    subject: "Reset Your Password — Launcherdesk CRM",
    html,
    text,
  });
}